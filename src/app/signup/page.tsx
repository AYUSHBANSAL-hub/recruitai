...
export default function SignUp() {
  ...
  const handleCheckboxArrayChange = (value, checked) => {
    setFormData({
      ...formData,
      recruitmentChallenges: checked
        ? [...formData.recruitmentChallenges, value]
        : formData.recruitmentChallenges.filter((item) => item !== value),
    })
  }
      ...
      <div className="relative h-[100px] mb-6">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            exit={{ opacity: 0, x: -50 }}
            transition={{ duration: 0.5 }}
            className={`absolute inset-0 bg-gradient-to-br ${stats[currentStat].bgClass} rounded-xl p-4 shadow-lg border`}
          >
